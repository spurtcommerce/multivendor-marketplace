...
function createEntitiesIndex() {
  ...
  const tmpFile = path.join(outDir, 'tmp-entities-index.ts');
  const outFile = path.join(outDir, 'entities-index.ts');
  if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir);
  }
  for (const item of fg.sync(path.join(src, 'api', '**', 'models', '**', '*.ts').replace(/\\/g, '/'))) {
  ...
  // addons
  for (const item of fg.sync(path.join(addOn, '**', 'models', '**', '*.ts').replace(/\\/g, '/'))) {
    const filePath = path.relative(outDir, item).replace(/\.ts$/, '').replace(/\\/g, '/');
    const data = `export * from '${filePath}';\n`;
    fs.writeFileSync(tmpFile, data, { flag: 'a+' });
  }
